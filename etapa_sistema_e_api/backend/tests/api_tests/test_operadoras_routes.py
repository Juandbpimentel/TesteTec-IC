...
@patch("backend.routes.operadoras_routes.get_operadoras")
def test_read_operadoras(mock_get_operadoras):
    mock_get_operadoras.return_value = {"operadoras": [], "total_elementos": 0}
    response = client.get("/api/operadoras/?limit=5")
    assert response.status_code == 200
    assert response.json() == {"operadoras": [], "total_elementos": 0}

@patch("backend.routes.operadoras_routes.get_maiores_despesas_trimestre")
...
@patch("backend.routes.operadoras_routes.get_maiores_despesas_ano")
...
@patch("backend.routes.operadoras_routes.get_ufs")
...
@patch("backend.routes.operadoras_routes.get_modalidades")
